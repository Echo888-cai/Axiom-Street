...
目前已补齐独立全库概览、验证最新记录与参考范围检查，以及过期心跳识别。扫描子任务的完整证据核验、真实环境回测与故障演练继续按 **P1 可靠性收口** 推进；可直接从[接手清单](docs/PLAN.md#11-交付状态与-deepseek-接手顺序)继续。
...
> **部署边界** · Compose 发布端口默认绑定 `127.0.0.1`，用于本地开发。当前尚无实际用户鉴权，不直接作为公网服务部署。真实密钥和研究运行产物留在本地或受控存储中。
