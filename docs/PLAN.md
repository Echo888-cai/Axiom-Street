...
| **P5-1** Copilot 底座 | 🔧 进行中(09-07 开工) | Phase 5 首个工作包(§8.2):AI 写路径隔离锁 + 只读上下文 API + provider 骨架(noop)+ 全局右栏确定性事实面板。产品拍板(09-07):底座先行、P5-2 接 Anthropic 直连、出站仅聚合统计、面板全局窄栏。无 LLM 调用、无聊天框、无写端点、无新表 |

---

## 3. 执行总览(RC-W2/3/4 与 EB-P5 已关闭;N5 = Phase 5,首个工作包 P5-1 09-07 开工)
...
| **P5-1** | Copilot 底座(§8.2,09-07 产品拍板):AI 写路径隔离锁 + 只读上下文 API + provider 骨架(noop)+ 全局右栏确定性事实面板 | 🔧 进行中(2026-09-07 开工) | Phase 5 首个工作包;无 LLM、无聊天、无写端点 |
...
| 包 | 内容 | 边界 |
|----|------|------|
| **P5-1** 底座(进行中 09-07) | AI 写路径代码级隔离(锁测试)+ 只读上下文 API + provider 适配器骨架(noop,env 门控)+ 全局右栏确定性事实面板(试验计数/闸门/重复参数/快照 supersede,诚实空态) | 无 LLM 调用、无聊天框、无写端点、无新表 |
...
| N5 | Phase 5 AI Copilot(§8.2:P5-1 底座 → P5-2 Anthropic 劝停 → P5-3 建议动作 → P5-4 聊天框待议) | P5-1 进行中(09-07 开工) | AI 加速研究(受闸门约束) |
| N6 | Phase 6/7 Paper+Live | +~16 周 | 研究到执行闭环 + 回测实盘对账(北极星可测) |
| N7 | Phase 8 组合归因 | +~20 周 | 多策略组合与真 alpha 判定 |

**如果只能做三件事**(09-07 随 EB-P5 关闭修订):① ~~Phase 5 前置工程债~~ —— **EB-P5 已关闭(09-07)**,Phase 5 门槛已清(nightly golden 的 CI 缺口为 §8.6 已知项);② **N5 Phase 5 AI Copilot**(受 §8.2 约束)——**P5-1 底座 09-07 开工进行中**(写路径隔离 + 只读上下文 + 右栏事实面板);③ N6 的回测–实盘对账(唯一能验证北极星的功能)。
