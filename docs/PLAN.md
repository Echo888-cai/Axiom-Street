...
| P2.2 任务中心 | **已交付** | 统一任务读模型 `GET /api/v1/tasks`、`GET /api/v1/tasks/{id}`、`POST /api/v1/tasks/{id}/cancel`（`services/api/services/tasks.py` + `routers/tasks.py`）：聚合 回测/验证/摄取/研究助手 四类台账为统一形状（kind/status/progress/ref/cancelable），按 created_at 倒序；取消语义明确（回测可取消并委派终态守卫；验证/摄取/助手 409“暂不支持取消”；未知 404）。前端 `features/tasks/task-drawer.tsx`：跨页任务抽屉（排队/运行/取消/失败回看、失败红点、10 秒轮询、取消后刷新），TopBar 铃铛由“仅回测”升级为统一抽屉。验收：关页重开可恢复（服务端读模型持久化）、网络重连不重提（抽屉只读轮询，不触发重投）、取消语义明确。验证：新增 3 后端 + 3 前端测试；`make test-all` 全过（后端 594 / 前端 141）。**下一步：P2.3 结果与比较** |
