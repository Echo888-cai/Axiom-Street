...
>
> 当前交付：文档归并、可信概览，以及 P1 验证与运行状态的首批修复。下一执行包：**P1.3b 扫描执行证据与试验代次**；P1 尚未整体验收。标为待做或未完成的任务不代表已经实现。
...
2026-09-14，在当前工作区执行 `make test-all` 通过；扫描输入继承修复后再次执行后端完整检查，最终结果：

- 后端：**550 项单元测试通过**；Ruff 检查和格式检查通过；mypy 检查 119 个源文件通过。
- 前端：**125 项测试通过**；TypeScript 检查通过；ESLint 无错误；生产构建通过。
- 仍有一个既有 LSP effect 清理告警，保留为 P1.4b；未通过屏蔽规则消除告警。
- 新增验收覆盖：137 条策略的全库计数、空库、列表外最新回测、零收益保留、新建/删除刷新、研究证据防误删、摘要失败隐藏缓存、最新失败/排队/运行覆盖旧成功、参考范围不一致、缺少快照/来源、缩短验证窗口、旧版本迟到回调、扫描参数继承、过期与未来心跳。
...
1. **扫描实际执行证据仍需核验。** 已修复 PBO/敏感性/成本扫描遗漏参考输入的问题，已核对各验证记录引用的参考回测；但引用相同模板不证明实际子回测采用相同引擎/数据。须继续检查 `result.backtest_ids`、Walk-Forward fold 产物及其执行输入。历史扫描缺少证明时必须判过期，不得套用新规则假定有效。
2. **家族试验代次尚未绑定。** 新增试验后的 DSR/SPA 等旧结论失效、并发汇总竞态尚未关闭。当前 `VALIDATED` 不代表这一层已经严密；Live 真实券商开关继续关闭。
...
| P1.3b 实际执行证据 | **下一包，最高优先级** | 核对扫描子回测与 fold 实际执行范围，见下方详细要求 |
| P1.3c 试验代次 | 待做 | 固定家族/快照试验集合签名；新增试验、并发更新撤销过期证据；前端显示原因和来源 |
