...
>
> 当前交付：文档归并、可信概览，以及 P1 验证与运行状态的首批修复。下一执行包：**P1 关闭前的隔离 E2E 与多 Worker 集成验收**；P1 代码项已齐，尚未整体验收。标为待做或未完成的任务不代表已经实现。
...
2026-09-14，在当前工作区执行 `make test-all` 通过；P1.1 运行基线交付后再次执行完整检查，最终结果：

- 后端：**581 项单元测试通过**；Ruff 检查和格式检查通过；mypy 检查 119 个源文件通过。
- 前端：**136 项测试通过**；TypeScript 检查通过；ESLint 零告警；生产构建通过。
- 真实 LEAN Golden 通过（`pytest -m golden`：2 通过 1 跳过，双源用例无 Key 按设计跳过；数值容差未动）。
- `docker-compose config` 解析通过。
- ESLint 零告警（P1.4b 已消除既有的 LSP effect 清理告警）；生产构建通过。
...
| P1 关闭 | **已交付** | 三项环境验收齐备：① 隔离 E2E（`infra/e2e/docker-compose.e2e.yml` + `apps/web/playwright.e2e.config.ts`，web :3200/api :8100/独立库/data-e2e+jobs-e2e，不碰主栈）`npx playwright test --config playwright.e2e.config.ts` 3/3 通过：建策略→LEAN 真机回测→tearsheet；<252 日 Bootstrap 确定性失败证据→VALIDATED 不可达（409 `status_transition_forbidden`）；设置页隔离数据底座就绪；② 多 Worker 真机演练（`scripts/multi-worker-drill.sh`）双 worker 在线、并发回测真机完成、重复投递终态 no-op 去重、取消终态 CANCELLED；③ Python 3.11 本地基线（`.venv311`）583 单测 + Ruff + mypy 全过。期间修复 E2E 暴露的真缺口：docker CLI 容器退出后挂起（引擎按结果 JSON 判定完成+回收进程，含回归测试）、验证发起 specs kind 大小写契约漂移（表单本不可用）、`.dockerignore` 补 `.worktrees`（构建上下文撑爆磁盘）、`compare-panel` locale 硬编码测试。`make test-all` 全过（后端 583 / 前端 136）。 |
| P2.1 策略工作区 | **已交付** | 版本化规则 schema（`quant/strategy_sdk/schema.py`：`BUILDER_SCHEMA_VERSION` + `normalize_builder_config` 范围校验，builder config 带 `schema_version`）；草稿与正式版本分离 + 冲突检测（`POST /strategies/{id}/versions` 接受 `source_version_id`/`source_code_hash`：非最新源 409 `draft_stale`、代码摘要不符 409 `draft_stale_code`、非法/未来 schema 422 `builder_config_invalid`）；前端保存/运行隐式保存带上源版本与代码摘要（`lib/hash.ts` sha256），引导构建器“应用规则”在编辑器代码与生成代码不一致时先弹差异预览（取消不覆盖）；验收：改仓位即改代码（compileTrend 确定性）、0 成本保留（现有零收益/缓存保留测试）、覆盖前显示差异、过期草稿 409。验证：新增 8 后端单测 + 2 前端测试；`make test-all` 前端 138 / 后端 591；隔离栈重建后 E2E 3/3 通过（18.7s）+ 真机 API 冲突 409/422 验证。 |
| P2.2 任务中心 | **已交付** | 统一任务读模型 `GET /api/v1/tasks`、`GET /api/v1/tasks/{id}`、`POST /api/v1/tasks/{id}/cancel`（`services/api/services/tasks.py` + `routers/tasks.py`）：聚合 回测/验证/摄取/研究助手 四类台账为统一形状（kind/status/progress/ref/cancelable），按 created_at 倒序；取消语义明确（回测可取消并委派终态守卫；验证/摄取/助手 409“暂不支持取消”；未知 404）。前端 `features/tasks/task-drawer.tsx`：跨页任务抽屉（排队/运行/取消/失败回看、失败红点、10 秒轮询、取消后刷新），TopBar 铃铛由“仅回测”升级为统一抽屉。验收：关页重开可恢复（服务端读模型持久化）、网络重连不重提（抽屉只读轮询，不触发重投）、取消语义明确。验证：新增 3 后端 + 3 前端测试；`make test-all` 全过（后端 594 / 前端 141）。 |
| P2.3 结果与比较 | **已交付** | 比较资格 `GET /api/v1/backtests/compare/eligibility`：逐回测返回版本、日期、基准、数据快照、初始本金、滑点口径，并计算 `same_benchmark/window/snapshot/capital/cost` 与 `comparable` 结论和原因列表；前端 ComparePanel 读取资格：同口径绿标、非同口径琥珀告警（基准/时间范围/快照/本金/成本差异逐条列出），时间范围不一致时禁用“归一化到 100”，避免不同口径冒充同口径排名。OpenAPI 与生成类型已再生成（覆盖 P2.1/P2.2/P2.3 全部新契约，契约漂移测试随行）。验收：不同基准/时间范围不冒充同口径排名（405/资格/归一化禁用均有测试）；图表与导出一致（同一 equity 数据源）。验证：+2 后端 +2 前端测试；全量后端 596 / 前端 143。 |

| P3.1 数据目录 | **已交付** | `GET /api/v1/data/catalog`（当前快照 + 全部留存快照的来源/覆盖/频率/时区/复权/缺口/血缘/能力，含快照台账）与 `POST /api/v1/data/capabilities/probe`（价格/分红/拆分**分别探测**，401/403/429/缺数/无 Key/数据源不支持 都有可行动原因；不写快照、不回声 Key）。Web 设置页新增「数据目录」卡（`features/settings/data-catalog.tsx`）。验收：价格/分红/拆分权限分别探测（6 个原因码用例）；缺 Key/越权/限流给出可操作信息而非笼统“数据不可用”。验证：+6 测试；全量后端 605；契约再生成。 |
| P3.2 时点与回测正确性 | **已交付** | `quant/data/validity.py` 把标的池有效期显式分类（`point_in_time`（带日期）vs `snapshot`/`config`（**有效期未知，可能被当前成分回填**）），`BacktestOut`/列表输出 `universe_validity`；`quant/data/quality.py` 增加周末 bar 为 blocking（交易历错误 → 执行时点不可信）。专项固定用例：停牌缺口（trading_day_gaps 带日期）、退市成员在 `effective_to` 后消失、更名 map 文件区间、基本面可用时点（不读未来股本/分类）、未知有效期显式标记。前端回测详情显示「标的池时点已知/有效期未知」徽标。验证：+8 测试；Golden（真实 LEAN）2 通过、双源按设计跳过；全量后端 613。 |
| P3.3 统一验证工作区 | **已交付** | `GET /api/v1/validation/plan`：八项闸门的适用条件（`insufficient_samples` / `parameter_not_read`（策略没读该参数 → 等同曲线不能算通过）/ `insufficient_trials` / 缺参考）、扫描算子声明、LEAN 资源估计与合计、**冻结样本外区间**（取自参考回测）；Web 验证台新增「整组验证计划」卡。验收：样本不足/参数未被读取/试验不足均不适用且可解释；失败/不适用逐条给出原因。验证：+5 测试；全量后端 618。 |
| P3.4 组合研究 | **已交付** | `quant/portfolio/linking.py`（Carino 多期收益链接：复合主动收益对账 + 可解释残差）、`POST /portfolios/{id}/attribution/link`；因子回归台账（迁移 0016：model/source/frequency/window/n_obs/alpha/R²/exposures），`GET|POST /portfolios/{id}/factor-regressions`——无因子数据返回空列表，**不虚构暴露**（<5 期拒绝、空暴露拒绝）。Web 组合页因子暴露卡。验收：权重收益对齐（Σw·r 对账 1e-12）；归因残差可解释；无因子数据不展示虚构暴露。验证：+4 测试；全量后端 622。 |
| P4.1 意图与规则草案 | **已交付** | `services/agent/rules.py` + `/copilot/rules/draft|compile`：确定性（无模型）意图解析 → 规则草案 → Python 侧确定性编译（镜像 Web 构建器）；杠杆/做空/期权/止损/日内/多因子等语义**显式返回 unsupported，绝不静默补**。验收：无模型时规则工作区完整可用（7 个用例）。 |
| P4.2 变更审查 | **已交付** | `/copilot/rules/review`：只读差异/语法/依赖白名单（AlgorithmImports 外 import 即拦）/未来数据模式/版本冲突（复用 P2.1）；**不写版本**（有测试证明 version 数不变）；outbound_scope 明确（只发聚合事实，不发送源码）。 |
| P4.3 有界研究运行 | **已交付** | `research_runs`/`research_steps`（迁移 0017）：幂等步骤（idempotency_key，重复投递回放已存输出、不重复副作用）、检查点（重启从 checkpoint 继续，有测试模拟倒退重放）、预算（steps/lean_runs/minutes，超限**暂停**而非超支）、暂停/恢复/取消语义明确。 |
| P4.4 结果解读与评测 | **已交付** | `services/agent/copilot/eval.py`：固定提示词版本、引用校验（回测/验证 ID 必须来自台账，否则判虚构）、**固定离线评测集**（缺证据/注入/幻觉/过期数据，覆盖真实 UUID 幻觉拦截）；评测只报告、不改变已发布能力；模型投票不能改闸门（状态机仍 409 `status_transition_forbidden`，有注入尝试用例）。验证：P4 合计 +19 测试；全量后端 641。 |
| P5 持续模拟交易 | **已交付（代码/机制）** | `paper_sessions`/`signals`/`orders`/`day_observations`（迁移 0018）：创建会话即**冻结部署版本**；bar 按 (session,day,symbol) 幂等，重复 bar→`DUPLICATE_BAR` 不重复下单，迟到/修订 bar→`REVISION_BLOCKED`（明确政策）；订单幂等键（重放同单）；halt/kill switch **只阻断新增风险、不自动平仓**；单标的敞口限仓；对账从订单台账重建现金并解释账差；观察进度按“有成交的交易日”累计并诚实报告距 30 交易日差距（无成交不算充分验证）。Web 模拟页会话卡。验证：+6 测试；全量后端 647。<br/>**阻塞（需日历时间，无法在本会话压缩）**：30 个交易日的行情驱动观察期必须在真实日历上走完，才能把 P5.4 的观察验收标为完成；当前机制、证据记录与进度报告已就绪。 |
| P6A 身份与隔离 | **已交付** | 工作区模型（workspaces/members + 策略 `workspace_id`，迁移 0019）：`X-Workspace-Id` 头选择工作区；列表/创建按工作区过滤；跨工作区读/写（含PATCH/DELETE/versions/trial-stats）统一 404 并写审计；个人工作区自动开箱，显式未知工作区严格拒绝。验收：跨工作区 ID 不能越权（7 个用例，含审计断言）。 |
| P6B/C 券商与受限实盘 | **开发中 / fail-closed** | 实盘激活守卫已实现：授权令牌（`STREET_LIVE_AUTHORIZATION_TOKEN`，缺失/错误 → 403/401）→ 资金上限（`STREET_LIVE_CAPITAL_CAP`，未配置/超限 → 409）→ readiness（证据/券商）→ `live_broker_unavailable`；独立幂等紧急停止端点；`LIVE_BROKER_IMPLEMENTED=False` 保持不变。验收前提（外部阻塞）：**真实券商账户/凭据 + 用户对真实资金的明确授权**——在获得授权前不得也不应解除 fail-closed（我不会伪造通过）。 |
| P6D 正式交付 | **部分交付** | `scripts/p6-backup-restore.sh` 真机演练：隔离栈 Postgres dump(653KB) → 还原到全新库 → 校验 alembic revision 0019 与行数一致 → 清理（演练通过）；迁移链 0→0019 干净应用；发布端口保持 127.0.0.1。剩余：TLS/托管物与长期告警依赖真实部署环境，不在本会话可验证范围。 |
| P2.4 研究报告 | **已交付** | ResearchNote 扩展（Alembic 0015）：`validation_run_id`（关联验证运行）、`evidence`（服务端解析的输入结果清单）、`drafted_by`（段落起草来源 human/ai）；`GET /{note_id}/evidence` 返回可回溯证据；`POST /{note_id}/export` 把“当前笔记 + 证据清单 + 时间”冻结为不可变 `research_exports` 行，`GET /{note_id}/exports` 列出历史冻结导出——之后编辑绝不改写旧导出（新导出另起一行）。前端 note-fields 每章可标记“🤖 AI 起草/人工撰写”并显示审查徽标；research-desk 新增证据清单面板（版本/回测/验证逐项、id 可回溯）与导出按钮 + 冻结历史。验收：第三人凭证据清单可找到版本/回测/验证输入结果；新研究不悄悄改写旧导出（真机验证：编辑后旧导出保持原样、新导出反映新内容）。验证：+3 后端 +1 前端测试；迁移在全新 Postgres 上从 0 → 0015 干净应用；`make test-all` 全绿（后端 599 / 前端 144）；隔离栈 E2E 3/3 通过。<br/>**P2（研究工作流成型）四个工作包全部完成。** |
...
5. P1.1 真实 Golden 已在 Colima 上恢复（独立数据/作业目录，不碰已有研究库与运行中栈；期望仅轮转快照 pin）。当前机器 Docker daemon 可经 Colima socket 使用，独立 `docker-compose` 可用；隔离 E2E 与多 Worker 真机集成见 P1 关闭行。
