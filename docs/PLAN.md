...
| 验证判定死代码(09-07 P5-1 开工审计发现;P5-3 后裁决清理) | `services/api/services/validation_spec.py` 原 `gate_check_for` 全仓无调用方(疑似死代码);实际判定来自 quant 层 `result.passed`,worker 直接写 run.passed | **已裁决删除(09-07 P5-3 后)**:先 grep 证实 services/quant/tests 全仓零调用、且该 wrapper 不在判定链入口(run.passed 由 quant `result.passed` 直写)后删除;`pytest tests/unit` 421 全绿。spec 抽象 `gate_check`(§6.2 spec 形态接口)保留不动 |
| 闸门口径文档漂移(09-07 P5-1 开工审计发现;P5-3 后修复) | `docs/validation-gates.md:137-143` 指向 `quant/validation/pbo.py`,实际实现是 `quant/metrics/pbo.py`(文档 PBO 小节路径不符) | **已修(09-07 P5-3 后)**:`validation-gates.md:139` PBO 核心实现改指 `quant/metrics/pbo.py`;对照实现复核其余 7 行路径全部正确,仅 PBO 一处漂移 |
