...
系统字体与卡片留白同步调整。没有新增依赖。后端此次仅审阅并编写交接计划。

本轮验证：121 项前端测试通过；ESLint 与生产构建（含 TypeScript 检查）通过。
桌面 1440px 检查概览、策略列表与规则编辑器；其余回测、验证、标的池、实验、笔记、
模拟交易、风控、组合和设置页面完成渲染与宽度检查。375px 手机检查首页、策略页、
导航与搜索弹层。所检查页面未见横向溢出。主文字、次文字、主按钮和盈亏语义色的
基础色对比达到 4.5:1；这不代表完成全站无障碍审计。
