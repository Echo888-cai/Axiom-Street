...
export function BootstrapForm() {
  ...
  const hint = useMemo(() => {
    ...
    if (!latest) return "先对该版本跑完一次全样本回测。Bootstrap 重抽样那条净值的日收益，不再跑 LEAN。";
    return "Stationary bootstrap 保留收益自相关。Sharpe 95% 区间下界 ≤ 0 则无统计显著性，不能进入 VALIDATED。需要至少 252 个交易日。";
  }, [items.length, latest]);

  const create = useMutation({
    mutationFn: () => {
      if (!versionId) throw new Error("没有可验证的策略版本");
      return api.createBootstrap({
  ...
  return (
    ...
    >
      ...
      <Button type="submit" disabled={!versionId || !latest || create.isPending}>
        {create.isPending ? "提交中…" : "运行 Bootstrap"}
      </Button>
    </form>
  );
}
