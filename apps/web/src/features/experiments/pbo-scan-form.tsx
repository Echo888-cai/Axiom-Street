...
function parseValues(raw: string): number[] {
  ...
  if (unique.length < 2 || unique.length > 12) {
    throw new Error("参数扫描需要 2–12 个互异正整数。");
  }
  return unique;
}
...
export function PboScanForm() {
  ...
  const [valuesRaw, setValuesRaw] = useState("100, 150, 200, 250");
  ...
  const hint = useMemo(() => {
    ...
    if (!hasLookback) {
      return "该版本没有读取 LEAN 参数 lookback。扫一个策略读不到的字段会得到相同净值，PBO 没有意义。请先提交新版本。";
    }
    if (!latest) {
      return "先对该版本跑完一次全样本回测。扫描借用那次回测的标的池与快照，每个 lookback 会写下真实回测与试验台账行。";
    }
    return "每个参数点是一次完整 LEAN 回测。CSCV 不会丢弃凑不齐切片的交易日。";
